...
namespace SharePoint.WebHooks.MVC.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            using (var cc = ContextProvider.GetWebApplicationClientContext(Settings.SiteCollection))
            {
                if (cc != null)
                {
                    ...
                    cc.Load(lists, l => l.Include(p => p.Title, p => p.Id, p => p.Hidden, p=>p.BaseTemplate));
                    ...
                    var filterList = lists.Where(l => l.BaseTemplate == 100).ToList();
                    foreach (var list in filterList)
                    {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<ActionResult> Create(string selectedSharePointList,string notificationUrl)
        {
            using (var cc = ContextProvider.GetWebApplicationClientContext(Settings.SiteCollection))
            {
                if (cc != null)
                {
                    ...
                    //http://sharepointwebhookclient.azurewebsites.net/api/spwebhook/handlerequest
                    WebHookManager webHookManager = new WebHookManager();
                    var res = await webHookManager.AddListWebHookAsync(cc.Web.Url, listId.ToString(), notificationUrl, this.accessToken);
                }
            }
        ...
        public async Task<ActionResult> Delete(string id = null, string listId = null)
        {
            using (var cc = ContextProvider.GetWebApplicationClientContext(Settings.SiteCollection))
            {
                if (cc != null)
                {
                    ...
                    WebHookManager webHookManager = new WebHookManager();
                    // delete the web hook
                    await webHookManager.DeleteListWebHookAsync(cc.Web.Url, listId, id, this.accessToken);
                }
            }
